...
import { Check, X, RefreshCw, Award, ArrowRight, Volume2, ShieldCheck, HelpCircle, Keyboard, Play, BookOpen, ChevronRight, HelpCircle as HintIcon } from 'lucide-react';
...
export default function Quiz({ words, onUpdateWordStatus }: QuizProps) {
  ...
  // Generate a quiz of 10 random questions according to chosen mode
  const generateQuiz = () => {
  ...
  return (
    <div className="w-full max-w-xl mx-auto flex flex-col gap-5 text-slate-800" id="quiz-panel">
        ...
        <div className="bg-white border border-slate-200 p-7 rounded-2xl flex flex-col gap-6 shadow-sm relative overflow-hidden" id="quiz-setup">
          ...
          <div className="flex flex-col gap-4">
            <label className="text-xs font-semibold text-slate-500 uppercase tracking-widest flex items-center gap-1.5">
              <span>Bước 1: Chọn Chế Độ Luyện Tập</span>
            </label>
            ...
            <div className="grid grid-cols-1 gap-3">
              ...
              <button
                ...
                className={`flex gap-4 p-4 text-left border rounded-xl transition cursor-pointer relative ${
              ...
              >
                <div className={`mt-0.5 rounded-full p-1.5 h-6 w-6 flex items-center justify-center text-xs font-bold leading-none ${
                  quizMode === 'pinyin-typing' ? 'bg-emerald-500 text-white' : 'bg-slate-200 text-slate-500'
                }`}>A</div>
              ...
              <button
                ...
                className={`flex gap-4 p-4 text-left border rounded-xl transition cursor-pointer relative ${
                  quizMode === 'meaning-typing'
                    ? 'border-amber-500 bg-amber-50 text-slate-800 shadow-sm'
                    : 'border-slate-200 bg-slate-50 hover:bg-slate-100 text-slate-500 hover:text-slate-800'
                }`}
              >
                <div className={`mt-0.5 rounded-full p-1.5 h-6 w-6 flex items-center justify-center text-xs font-bold leading-none ${
                  quizMode === 'meaning-typing' ? 'bg-amber-500 text-white' : 'bg-slate-200 text-slate-500'
                }`}>B</div>
                <div className="flex-1 flex flex-col gap-0.5">
              ...
              <button
                ...
                className={`flex gap-4 p-4 text-left border rounded-xl transition cursor-pointer ${
              ...
              >
                <div className={`mt-0.5 rounded-full p-1.5 h-6 w-6 flex items-center justify-center text-xs font-bold leading-none ${
                  quizMode === 'multiple-choice' ? 'bg-blue-500 text-white' : 'bg-slate-200 text-slate-500'
                }`}>C</div>
                <div className="flex-1 flex flex-col gap-0.5">
          ...
          <div className="flex items-center gap-2.5 mt-2 text-xs text-slate-500 bg-slate-50 p-3.5 rounded-xl border border-slate-200">
            ...
            <span>
              Yêu cầu có tối thiểu <strong className="text-slate-800">4 từ vựng</strong> trong danh bạ để soạn câu hỏi. Hiện bộ từ đã nạp: <strong className="text-slate-800">{words.length} từ</strong>.
            </span>
          </div>
          ...
          <button
            ...
            className={`w-full py-4.5 rounded-xl font-bold text-sm shadow opacity-95 transition-all flex items-center justify-center gap-2 cursor-pointer ${
              words.length >= 4
          ...
          >
            ...
            <span>Bắt đầu ôn luyện (10 câu ngẫu nhiên)</span>
          </button>
        </div>
            ...
            <div className="h-2 w-1/3 bg-slate-100 rounded-full overflow-hidden mx-3">
              <div 
                className={`h-full transition-all duration-300 ${
                  quizMode === 'pinyin-typing' ? 'bg-emerald-500' : quizMode === 'meaning-typing' ? 'bg-amber-400' : 'bg-blue-500'
                }`}
          ...
          <div className="flex flex-col gap-2.5" id="question-prompt-layout">
            ...
            <div className="mt-2 p-5 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between gap-4">
              ...
              <div className="flex flex-col items-center gap-1 shrink-0 bg-white border border-slate-200 p-3 rounded-xl min-w-[70px] shadow-sm">
                <span className="text-4xl font-sans tracking-wide text-slate-800 font-medium">{activeQuestion.word.character}</span>
                <button
              ...
              <div className="flex flex-col gap-3.5">
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full py-3.5 px-4 rounded-xl border font-medium text-sm transition focus:outline-none focus:ring-1 ${
                      isSubmitted
                        ? isCorrectResult
          ...
          <AnimatePresence>
            {isSubmitted && (
              <motion.div
                ...
                exit={{ opacity: 0, y: 10 }}
                className={`p-4 rounded-xl border flex flex-col gap-1.5 shadow-sm ${
                  isCorrectResult
            ...
            <div className="flex gap-2">
              {!isSubmitted ? (
                <button
                  ...
                  className={`px-6 py-2.5 rounded-xl text-xs font-bold transition flex items-center gap-1 shadow cursor-pointer ${
                    (quizMode === 'multiple-choice' ? selectedOption : userTypedInput.trim())
                      ? 'bg-slate-900 text-white hover:bg-slate-800 active:scale-95'
                      : 'bg-slate-100 text-slate-400 cursor-not-allowed border border-slate-200'
                  }`}
                >
